...
```
줄 1: [메커니즘] — 현재 구조가 왜 이 문제를 만드는가
줄 2: [필요한 것의 방향] — 이 메커니즘을 해결하려면 어떤 종류의 것이 필요한가
줄 3: [선택의 근거] — 여러 대안 중 왜 이 Epic의 방향인가
줄 4: [구체적 출발점] — 지금 당장 존재하는 기회·사례·파트너
```

각 줄은 이전 줄에서 논리적으로 이어져야 합니다. 마지막 줄은 Roadmap의 Phase 1으로 자연스럽게 연결되어야 합니다.

피할 것:
- **단순 정보 나열**: 각각 다른 각도의 사실들을 쌓는 것. 독자가 "그래서 왜 이 방향인가"를 스스로 추론해야 한다면 논리 흐름이 부족한 것입니다.
